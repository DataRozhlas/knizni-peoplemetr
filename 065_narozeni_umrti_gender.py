...
aut = aut.explode("375_a").with_columns(
    pl.when(pl.col("375_a") == "muž")
    .then(pl.lit("m"))
    .when(pl.col("375_a") == "žena")
    .then(pl.lit("f"))
    .otherwise(None)
    .alias("gender")
)
...
aut = (
    aut.explode("100_d")
    .with_columns(pl.col("100_d").map_elements(roky_z_autorit).alias("letopocty"))
    .unnest("letopocty")
)

aut = aut.explode("678_a").with_columns(
    pl.col("678_a").map_elements(lambda x: x.split(" ")[0] if x else None).alias("bio1")
)

muzska_bio1 = (
    aut.filter(pl.col("gender") == "m")
    .group_by(["bio1"])
    .len()
...
aut_zmeneno = aut_budeme_menit.with_columns(
    pl.when(pl.col("100_7").is_in(predelat_na_muze))
    .then(pl.lit("m"))
    .when(pl.col("100_7").is_in(predelat_na_zeny))
    .then(pl.lit("f"))
    .otherwise(None)
    .alias("gender")
)
...
aut_vyplnene = aut_spojene.filter(
    ~pl.col("narozeni").is_null()
    | ~pl.col("umrti").is_null()
    | ~pl.col("umrti").is_null()
).select(pl.col(["100_7", "narozeni", "umrti", "gender"]))


wikid = wikid.with_columns(
    pl.when(pl.col("w_gender") == "muž")
    .then(pl.lit("m"))
    .when(pl.col("w_gender") == "žena")
    .then(pl.lit("f"))
    .otherwise(None)
    .alias("gender")
)
...
wikid_vyplnene = (
    wikid.with_columns(pl.col("w_narozeni").cast(int).alias("narozeni"))
    .with_columns(pl.col("w_umrti").cast(int).alias("umrti"))
    .rename({"__index_level_0__": "100_7"})
    .select(pl.col(["100_7", "narozeni", "umrti", "gender"]))
    .filter(
        ~pl.col("narozeni").is_null()
        | ~pl.col("umrti").is_null()
...
def specialized_join(df1: pl.DataFrame, df2: pl.DataFrame) -> pl.DataFrame:
    # Perform outer join
    joined = df1.join(df2, on="100_7", how="outer", suffix="_df2")
...
df = df.with_columns(
    ...
    pl.when(~pl.col("doziti").is_between(15, 115))
    .then(None)
    .otherwise(pl.col("umrti"))
    .alias("umrti")
)
