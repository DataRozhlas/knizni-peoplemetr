...
martinus_sledovat = (
    pd.read_json(
        os.path.join(
            "/mnt/usbdrive/knizni-peoplemetr/data_raw", "martinus_sledovat.json"
        ),
        dtype={"M_isbn": str},
    )["M_isbn"]
    .drop_duplicates()
...
try:
    with open(
        os.path.join("/mnt/usbdrive/knizni-peoplemetr/data_raw", "cnb_sledovat.json"),
        "r",
    ) as cnb_sledovat:
...
try:
    with open(
        os.path.join("/mnt/usbdrive/knizni-peoplemetr/data_raw", "urls_sledovat.json"),
        "r",
    ) as urls_sledovat:
...
with open(
    os.path.join("/mnt/usbdrive/knizni-peoplemetr/data_raw", "sledovat.json"), "w+"
) as json_file:
