            ...
            record_data = {
                ...
                "autorstvo_kod": next(
                    (
                        ...
                        for subfield in field.findall('.//marc:subfield[@code="7"]', NS)
                    ),
                    None,
                ),
                "autorstvo": next(
                    (
                        subfield.text
                        for field in elem.findall('.//marc:datafield[@tag="100"]', NS)
                        for subfield in field.findall('.//marc:subfield[@code="a"]', NS)
                    ),
                    None,
                ),
                "titul": next(
...
for record in stream_marc_records("downloads/skc.xml"):
    ...
    if len(current_batch) >= batch_size:
        ...
        con.execute(
            "INSERT INTO marc_records (id, titul, autorstvo, autorstvo_kod) SELECT id, titul, autorstvo, autorstvo_kod FROM arrow_table"
        )
        current_batch = []
