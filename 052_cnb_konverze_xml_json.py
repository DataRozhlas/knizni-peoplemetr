                        ...
                        if isinstance(podhodnota, str) and podhodnota.strip():
                            ...
                            for posledni_uroven in podhodnota:
                                for (
                                    posledni_radek,
                                    posledni_hodnota,
                                ) in posledni_uroven.items():
                                    mistni_pismena.remove(posledni_radek)
                                    if (posledni_radek not in mistni_pismena) or (
                                        "700" not in radek
                                    ):
                                        heslo[f"{radek}_{posledni_radek}"].append(
                                            posledni_hodnota
                                        )
                            if radek in list(doplneni.keys()):
                                cotuje = [
                                    list(podhodnoty.keys())[0]
                                    for podhodnoty in podhodnota
                                ]
...
for file_code in kody:
    ...
    vsechna_xml = [
        f
        for f in os.listdir(f"downloads/{file_code}")
        if (f[0:3] == file_code) and ("_" in f)
    ]
    ...
    for v in vsechna_xml:
        ...
        with open(
            os.path.join(kam_s_tim, v.replace(".xml", ".json")), "w+", encoding="utf-8"
        ) as vystup:
    ...
    with open(
        os.path.join("data_raw", f"{file_code}_vsechny_sloupce.json"),
        "w+",
        encoding="utf-8",
    ) as seznam_sloupcu:
    ...
    print(
        f"Ve zdroji {file_code} nalezeno {len(vsechny_sloupce)} sloupců, uloženy do JSONu."
    )
