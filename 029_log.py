...
log_file_name = (
    f"log_knizni_peoplemetr_{current_date.replace(' ', '_').replace(':', '-')}.txt"
)

# Open the log file in write mode
with open(os.path.join("logs", log_file_name), "w") as f:
    f.write(
        f"Log knižního peoplemetru z {current_date}\n==============================================\n"
    )
    ...
    f.write(
        str(
            df.groupby(pd.Grouper(key="GR_date", freq="W"))["GR_isbn"]
            .nunique()
            .tail(3)
            .iloc[::-1]
        )
    ...
    f.write(
        str(
            df.groupby(pd.Grouper(key="DK_date", freq="W"))["DK_isbn"]
            .nunique()
            .tail(3)
            .iloc[::-1]
        )
