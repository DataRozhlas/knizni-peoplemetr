...
def rozsekej(frame, cislo):
    ...
    for c in cisla_komplet:
        ...
        selected_columns.columns = selected_columns.columns.astype(
            str
        )  ## aby zůstaly nuly na začátku názvů sloupců
...
try:
    with open(
        os.path.join("data_raw", "cnb_explodovatelne_sloupce.json"),
        "r",
        encoding="utf-8",
    ) as file:
...
if filtr == True:
    ...
    with open("readme.md", "r", encoding="utf-8") as readme:
        readme = (
            readme.read().split("# Klíč k")[-1].split("Databáze národních autorit")[0]
        )
...
if autority == True:
    ...
    for j in jsony:
        ...
        sloupce_k_zachovani_filtr = [
            s for s in sloupce_k_zachovani if s in df.columns.to_list()
        ]
...
if rozsekani == True:
    ...
    for c in columns:
        ...
        s = pd.DataFrame()
        for p in [
            d for d in os.listdir(f"{kam_rozsekane_sloupce}/{c}") if "parquet" in d
        ]:
            s = pd.concat(
        ...
        ]  ## PROVIZORNÍ FIX, nutno překontrolovat, proč tam jsou 2×
