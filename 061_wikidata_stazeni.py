...
def get_wikidata_json(
    entity_id: str, kam: str, retries: int = 3, initial_delay: int = 5
) -> bool:
    """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    logging.error(f"Failed to write file for {entity_id}: {e!s}")
                    time.sleep(2)
                    continue
                ...
                logging.warning(
                    f"Failed to download {entity_id}: Status {response.status_code}"
                )
            ...
            delay = initial_delay * (2**attempt)
            logging.warning(f"Request error for {entity_id}: {e!s}")
            time.sleep(delay)
            continue

        except Exception as e:
            logging.error(f"Unexpected error downloading {entity_id}: {e!s}")
...
def load_existing_downloads(kam: str) -> set[str]:
    ...
    try:
        return set(
            f.replace(".json", "") for f in os.listdir(kam) if f.endswith(".json")
        )
    except OSError as e:
        logging.error(f"Error reading directory {kam}: {e!s}")
        return set()
...
def process_batch(entities: list[str], kam: str, batch_size: int = 100) -> None:
    ...
    for i in range(0, len(entities), current_batch_size):
        ...
        logging.info(
            f"Processing batch {i // current_batch_size + 1}, size: {current_batch_size}"
        )
        ...
        try:
            ...
            if failure_rate > 0.5:
                ...
                logging.info(
                    f"High failure rate detected, reducing batch size to {current_batch_size}"
                )
                time.sleep(30)
            elif current_batch_size < batch_size:
                current_batch_size = min(batch_size, current_batch_size * 2)
                logging.info(
                    f"Success rate improved, increasing batch size to {current_batch_size}"
                )

            logging.info(f"Batch complete: {successful}/{len(batch)} successful")

        except Exception as e:
            logging.error(f"Batch processing error: {e!s}")
            current_batch_size = max(10, current_batch_size // 2)
            time.sleep(60)
            continue
...
def process_wikidata_ids(autority: pd.DataFrame) -> list[str]:
    ...
    try:
        aut_wikids = autority[autority["024_2"] == "wikidata"]["024_a"].drop_duplicates(
            keep="first"
        )
        ...
        try:
            ...
            print(
                f"V předchozím kroku doplněno extra {len(chybejici)} ids přes hledání na https://query.wikidata.org/sparql"
            )
        except OSError:
            chybejici = []
        ...
        for c in chybejici:
            if "Q" in c.split("_")[1]:
                doplneni.append(
                    {"001": c.split("_")[0], "024_a": c.split("_")[1].split(".")[0]}
                )

        wikiny_audiny = pd.concat(
            [pd.DataFrame(aut_wikids), pd.DataFrame(doplneni).set_index("001")]
        )
        ...
        logging.error(f"Error processing Wikidata IDs: {e!s}")
        raise
...
def main():
    try:
        ...
        logging.error(f"Critical error in main: {e!s}")
        raise
