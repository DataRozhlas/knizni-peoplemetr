...
def dopln_novinky(slovnik):
    ...
    def knihy_do_souboru(s, sez):
        ...
        with open(
            os.path.join("/mnt/usbdrive/knizni-peoplemetr/data_raw/martinus_linky", s),
            "w+",
            encoding="utf-8",
        ) as f1:
            f1.write("\n".join(sez))
    ...
    def pavouk(s, u):
        ...
        try:
            with open(
                os.path.join(
                    "/mnt/usbdrive/knizni-peoplemetr/data_raw/martinus_linky", s
                ),
                "r",
                encoding="utf-8",
            ) as f2:
        ...
        print(
            f"Končím na straně {x}, {nove} nových knih, {kontrola} knih se opakovalo."
        )
