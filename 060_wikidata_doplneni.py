...
autority_s_wikidaty = set(
    autority[autority["024_2"] == "wikidata"]["024_a"].index.to_list()
)
...
def process_authority(nl_cr_aut_id):
    ...
    # Save the result
    with open(
        os.path.join(kam, f"{nl_cr_aut_id}_{slovnik['024_a']}.json"),
        "w",
        encoding="utf-8",
    ) as file:
...
print(
    f"Zjistím Wikidata IDs pro {total_to_process} personálních autorit pomocí paralelního zpracování."
)
...
for i in range(0, total_to_process, BATCH_SIZE):
    ...
    print(
        f"Zpracovávám dávku {i // BATCH_SIZE + 1}/{(total_to_process + BATCH_SIZE - 1) // BATCH_SIZE}"
    )

    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        # Use tqdm for progress tracking
        future_to_auth = {
            executor.submit(process_authority, auth_id): auth_id for auth_id in batch
        }

        for future in tqdm(
            concurrent.futures.as_completed(future_to_auth), total=len(batch)
        ):
